...
public:
    ...
    UnitConversion resolve_conversion(const std::string &from_symbol, const std::string &to_symbol) const;

    /// Returns true when both symbols exist and share the same dimension vector.
    bool are_dimensionally_compatible(const std::string &lhs_symbol, const std::string &rhs_symbol) const;
