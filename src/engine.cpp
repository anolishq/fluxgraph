...
void Engine::load(CompiledProgram program) {
    ...
    if (required_command_capacity_ > 0 &&
        required_command_capacity_ <= std::numeric_limits<size_t>::max() / kCommandBacklogTicks) {
...
void Engine::tick(double dt, SignalStore &store) {
    ...
    if (required_signal_capacity_ > 0 && store.capacity() < required_signal_capacity_) {
    ...
    for (const auto &model : models_) {
        ...
        if (dt > limit) {
            throw std::runtime_error("Engine: stability violation for model '" + model->describe() +
                                     "' (dt=" + std::to_string(dt) + " exceeds limit=" + std::to_string(limit) + ")");
        }
    }
