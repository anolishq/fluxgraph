...
ParamValue json_to_param_value(const json &j, const std::string &path, detail::ParamParseBudget &budget,
                               size_t depth = 0) {
    ...
    if (j.is_number_float()) {
        ...
        for (size_t i = 0; i < j.size(); ++i) {
            arr.push_back(json_to_param_value(j[i], path + "/" + std::to_string(i), budget, depth + 1));
        }
        return arr;
    } else if (j.is_object()) {
        ...
        throw std::runtime_error("Parameter parse error at " + path + ": Unsupported JSON type");
    }
}
...
TransformSpec parse_transform(const json &j, const std::string &base_path, detail::ParamParseBudget &budget) {
    TransformSpec spec;
...
SignalSpec parse_signal(const json &j, const std::string &base_path, size_t index) {
    const std::string path = base_path + "/" + std::to_string(index);
    SignalSpec spec;

    if (!j.contains("path")) {
        throw std::runtime_error("JSON parse error at " + path + ": Missing required field 'path'");
    }
    if (!j.contains("unit")) {
        throw std::runtime_error("JSON parse error at " + path + ": Missing required field 'unit'");
    }
...
EdgeSpec parse_edge(const json &j, const std::string &base_path, size_t index, detail::ParamParseBudget &budget) {
    EdgeSpec spec;
    ...
    if (!j["transform"].is_object()) {
        throw std::runtime_error("JSON parse error at " + path + "/transform: Expected object");
    }
...
ModelSpec parse_model(const json &j, const std::string &base_path, size_t index, detail::ParamParseBudget &budget) {
    ModelSpec spec;
...
GraphSpec parse_json(const json &j) {
    ...
    if (j.contains("models") && j["models"].is_array()) {
        ...
        for (auto &model_json : j["models"]) {
            spec.models.push_back(parse_model(model_json, "/models", index++, param_budget));
        }
    }
    ...
    if (j.contains("edges") && j["edges"].is_array()) {
        ...
        for (auto &edge_json : j["edges"]) {
            spec.edges.push_back(parse_edge(edge_json, "/edges", index++, param_budget));
        }
    }
