...
std::string format_yaml_error(const std::string &message, const YAML::Mark &mark) {
    return "YAML parse error at line " + std::to_string(mark.line + 1) + ", column " + std::to_string(mark.column + 1) +
           ": " + message;
}
...
bool parse_int64_strict(const std::string &text, int64_t &out) {
    ...
    if (parsed < std::numeric_limits<int64_t>::min() || parsed > std::numeric_limits<int64_t>::max()) {
        return false;
    }
...
ParamValue yaml_to_param_value(const YAML::Node &node, const std::string &path, detail::ParamParseBudget &budget,
                               size_t depth = 0) {
...
TransformSpec parse_transform(const YAML::Node &node, const std::string &base_path, detail::ParamParseBudget &budget) {
    TransformSpec spec;
    ...
    if (node["params"]) {
        if (!node["params"].IsMap()) {
            throw std::runtime_error("YAML parse error at " + path + "/params: Expected map");
        }
...
EdgeSpec parse_edge(const YAML::Node &node, size_t index, detail::ParamParseBudget &budget) {
    ...
    if (!node["transform"]) {
        throw std::runtime_error("YAML parse error at " + path + ": Missing required field 'transform'");
    }
    if (!node["transform"].IsMap()) {
        throw std::runtime_error("YAML parse error at " + path + "/transform: Expected map");
    }
...
SignalSpec parse_signal(const YAML::Node &node, size_t index) {
    ...
    if (!node["path"]) {
        throw std::runtime_error("YAML parse error at " + path + ": Missing required field 'path'");
    }
    if (!node["unit"]) {
        throw std::runtime_error("YAML parse error at " + path + ": Missing required field 'unit'");
    }
...
ModelSpec parse_model(const YAML::Node &node, size_t index, detail::ParamParseBudget &budget) {
        ...
        for (size_t i = 0; i < node["actions"].size(); ++i) {
            ...
            if (action_node["args"]) {
                if (!action_node["args"].IsMap()) {
                    throw std::runtime_error("YAML parse error at " + action_path + "/args: Expected map");
                }
