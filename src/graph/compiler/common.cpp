...
const ParamValue &require_param(const ParamMap &params, const std::string &name, const std::string &context) {
    ...
    if (it == params.end()) {
        throw std::runtime_error("Missing required parameter at " + context + "/" + name);
    }
    return it->second;
}

double as_double(const ParamValue &value, const std::string &path) { return param::as_double(value, path); }

int64_t as_int64(const ParamValue &value, const std::string &path) { return param::as_int64(value, path); }

bool as_bool(const ParamValue &value, const std::string &path) { return param::as_bool(value, path); }

std::string as_string(const ParamValue &value, const std::string &path) { return param::as_string(value, path); }

const ParamArray &as_array(const ParamValue &value, const std::string &path) { return param::as_array(value, path); }

const ParamObject &as_object(const ParamValue &value, const std::string &path) { return param::as_object(value, path); }

void require_finite(const double value, const std::string &path) {
    if (!std::isfinite(value)) {
        throw std::runtime_error("Invalid parameter at " + path + ": expected finite number");
    }
}
...
std::string format_scalar_constraint_rule(const ScalarConstraint &constraint) {
    switch (constraint.kind) {
        ...
        case ScalarConstraint::Kind::closed_interval:
            return "in [" + std::to_string(constraint.a) + ", " + std::to_string(constraint.b) + "]";
    }
    return "finite number";
}

bool satisfies_scalar_constraint(const double value, const ScalarConstraint &constraint) {
    if (!std::isfinite(value)) {
        return false;
    }
    ...
    switch (constraint.kind) {
        ...
        case ScalarConstraint::Kind::greater_equal:
            return value >= constraint.a;
        case ScalarConstraint::Kind::closed_interval:
            return constraint.a <= constraint.b && value >= constraint.a && value <= constraint.b;
    }
    return false;
}
...
std::function<bool(const SignalStore &)> compile_condition_expr(const std::string &expr, SignalNamespace &signal_ns,
    ...
    if (!std::regex_match(trimmed, match, rule_comparator_regex())) {
        throw std::runtime_error("Unsupported rule condition syntax for rule '" + rule_id +
    ...
    if (op == "<") {
        return [signal_id, rhs](const SignalStore &store) { return store.read_value(signal_id) < rhs; };
    }
    if (op == "<=") {
        return [signal_id, rhs](const SignalStore &store) { return store.read_value(signal_id) <= rhs; };
    }
