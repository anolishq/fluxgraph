...
void SignalStore::write(SignalId id, double value, const std::string &unit) {
    ...
    if (has_declared_unit_[index] && declared_units_[index] != normalized_unit) {
        throw std::runtime_error("Unit mismatch for signal " + std::to_string(id) + ": expected '" +
                                 declared_units_[index] + "', got '" + normalized_unit + "'");
    }
...
void SignalStore::write_with_source_unit(SignalId target, double value, SignalId source) {
    ...
    const bool source_written = source_index < signals_.size() && has_signal_[source_index] != 0;
...
void SignalStore::declare_unit(SignalId id, const std::string &expected_unit) {
    ...
    ensure_index(id);
    const size_t index = static_cast<size_t>(id);
    const std::string &normalized_unit = expected_unit.empty() ? dimensionless_unit() : expected_unit;

    if (has_declared_unit_[index] != 0U && declared_units_[index] != normalized_unit) {
        throw std::runtime_error("Signal unit contract redefinition for signal " + std::to_string(id) + ": existing '" +
                                 declared_units_[index] + "', new '" + normalized_unit + "'");
    }
