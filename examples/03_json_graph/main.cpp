...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (heater_id != fluxgraph::INVALID_SIGNAL && ambient_id != fluxgraph::INVALID_SIGNAL &&
            chamber_id != fluxgraph::INVALID_SIGNAL && display_id != fluxgraph::INVALID_SIGNAL) {
            ...
            for (int i = 0; i <= 100; ++i) {
                ...
                if (i % 10 == 0) {
                    ...
                    std::cout << std::fixed << std::setprecision(1) << std::setw(7) << i * dt << "  " << std::setw(9)
                              << heater << "  " << std::setw(11) << chamber << "  " << std::setw(11) << display << "\n";
                }
            ...
            if (u_id == fluxgraph::INVALID_SIGNAL || y_id == fluxgraph::INVALID_SIGNAL) {
                throw std::runtime_error("Example runner does not recognize graph IO signals");
            }
            ...
            for (int i = 0; i <= 80; ++i) {
                ...
                if (i % 8 == 0) {
                    ...
                    std::cout << std::fixed << std::setprecision(2) << std::setw(7) << t << "  " << std::setw(8) << u
                              << "  " << std::setw(9) << y << "\n";
                }
            }
        }
