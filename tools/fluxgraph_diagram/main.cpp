...
CliArgs parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (token == "--in" || token == "--out" || token == "--format" || token == "--dot-bin" ||
            token == "--dot-out") {
            if (i + 1 >= argc) {
    ...
    if (!parsed_format.has_value()) {
        throw std::runtime_error("Unsupported format: " + format_value + ". Valid values: dot, svg, png.");
    }
    args.output_format = parsed_format.value();

    if (args.output_format == fluxgraph::viz::OutputFormat::Dot && !args.dot_output_path.empty() &&
        args.dot_output_path != args.output_path) {
        throw std::runtime_error("For --format dot, --dot-out must match --out when provided.");
    }

    if (args.output_format != fluxgraph::viz::OutputFormat::Dot && !args.dot_output_path.empty() &&
        args.dot_output_path == args.output_path) {
        throw std::runtime_error("For image outputs, --dot-out must differ from --out.");
    }
...
#else
        throw std::runtime_error("JSON input requested, but FLUXGRAPH_JSON_ENABLED is OFF.");
#endif
    }
...
#else
        throw std::runtime_error("YAML input requested, but FLUXGRAPH_YAML_ENABLED is OFF.");
#endif
    }

    throw std::runtime_error("Unsupported input extension '" + extension + "'. Use .json, .yaml, or .yml input files.");
}
...
std::string create_temp_dot_path() {
    ...
    for (int attempt = 0; attempt < 64; ++attempt) {
        const std::string filename =
            "fluxgraph-diagram-" + std::to_string(now_ticks) + "-" + std::to_string(attempt) + ".dot";
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (const auto &type : extension_types) {
            std::cerr << "warning: extension transform type '" << type << "' rendered as annotation.\n";
        }
