...
constexpr std::array<std::string_view, 8> kBuiltinTransformTypes = {
    "linear", "first_order_lag", "delay", "noise", "saturation", "deadband", "rate_limiter", "moving_average"};
...
bool is_builtin_transform_type(const std::string &transform_type) {
    return std::find(kBuiltinTransformTypes.begin(), kBuiltinTransformTypes.end(), transform_type) !=
           kBuiltinTransformTypes.end();
}
...
std::string quote_dot_string(const std::string &value) { return "\"" + escape_dot_string(value) + "\""; }
    ...
    return std::visit(
        [](const auto &item) -> std::string {
            ...
            if constexpr (std::is_same_v<T, double>) {
                ...
                static_assert(std::is_same_v<T, ParamObject>, "Unhandled ParamValue variant alternative");
                std::string rendered = "{";
                bool first = true;
...
std::string emit_dot(const GraphSpec &spec, const DotEmitOptions &options) {
    ...
    for (const auto &signal_id : signal_ids) {
        nodes.push_back(NodeRecord{signal_id, signal_id.substr(std::string("signal:").size()), "ellipse"});
    }
    ...
    if (options.include_rules) {
        for (const auto &rule : spec.rules) {
            nodes.push_back(NodeRecord{"rule:" + rule.id, format_rule_label(rule), "note"});
        }
    }

    std::sort(nodes.begin(), nodes.end(), [](const NodeRecord &lhs, const NodeRecord &rhs) { return lhs.id < rhs.id; });

    std::sort(edges.begin(), edges.end(), [](const EdgeRecord &lhs, const EdgeRecord &rhs) {
        return std::tie(lhs.source_id, lhs.target_id, lhs.transform_type, lhs.transform_label) <
               std::tie(rhs.source_id, rhs.target_id, rhs.transform_type, rhs.transform_label);
    });
    ...
    for (const auto &edge : edges) {
        ...
        attributes["transform_type"] = edge.transform_type.empty() ? "transform" : edge.transform_type;

        dot << "  " << quote_dot_string(edge.source_id) << " -> " << quote_dot_string(edge.target_id)
