...
std::string build_graphviz_command(const GraphvizRenderRequest &request) {
    ...
    command << shell_quote(request.dot_binary) << " -T" << output_format_name(request.output_format) << " "
            << shell_quote(request.dot_input_path) << " -o " << shell_quote(request.output_path);
    return command.str();
}
...
GraphvizRenderResult render_with_graphviz(const GraphvizRenderRequest &request) {
    ...
    const intptr_t spawn_result = _spawnvp(_P_WAIT, request.dot_binary.c_str(), argv.data());
    if (spawn_result == static_cast<intptr_t>(-1)) {
        char error_buffer[128] = {};
        strerror_s(error_buffer, sizeof(error_buffer), errno);
        std::ostringstream message;
        message << "Failed to launch Graphviz renderer '" << request.dot_binary << "': " << error_buffer;
