...
double analytical_temperature(const ScenarioConfig &scenario, double t) {
    ...
    const double steady_state = scenario.ambient_temp + scenario.power / scenario.heat_transfer_coeff;
    return steady_state + (scenario.initial_temp - steady_state) * std::exp(-k * t);
}

ValidationPoint run_point(const ScenarioConfig &scenario, ThermalIntegrationMethod method, double dt,
                          double duration_s) {
    const std::size_t steps = static_cast<std::size_t>(std::llround(duration_s / dt));
    if (steps == 0) {
    ...
    ThermalMassModel model("validation", scenario.thermal_mass, scenario.heat_transfer_coeff, scenario.initial_temp,
...
MethodResult run_method(const ScenarioConfig &scenario, ThermalIntegrationMethod method,
                        const std::vector<double> &dt_values, double duration_s) {
    MethodResult result;
    ...
    for (const auto &scenario : results) {
        for (const auto &method : scenario.methods) {
            for (const auto &point : method.points) {
                out << scenario.config.id << "," << method.method << "," << point.dt << "," << point.steps << ","
                    << point.l2_error << "," << point.linf_error << "," << point.final_abs_error << "\n";
            }
        }
    }
}

void write_json(const std::string &path, double duration_s, const std::vector<double> &dt_values,
    ...
    for (const auto &scenario : results) {
        for (const auto &method : scenario.methods) {
            if (method.method == "forward_euler" && method.observed_order_linf < min_euler_order_linf) {
                min_euler_order_linf = method.observed_order_linf;
            } else if (method.method == "rk4" && method.observed_order_linf < min_rk4_order_linf) {
...
int main(int argc, char **argv) {
    ...
    for (const auto &scenario : scenarios) {
        ...
        scenario_result.methods.push_back(
            run_method(scenario, ThermalIntegrationMethod::ForwardEuler, opts.dt_values, opts.duration_s));
        scenario_result.methods.push_back(
            run_method(scenario, ThermalIntegrationMethod::Rk4, opts.dt_values, opts.duration_s));
