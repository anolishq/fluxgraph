...
public:
    ...
    void reset() override {}
    ITransform *clone() const override { return new DeterministicCustomTransform(*this); }
...
void register_custom_deterministic_transform() {
    ...
    GraphCompiler::register_transform_factory(type, [](const TransformSpec &spec) -> std::unique_ptr<ITransform> {
        ...
        return std::make_unique<DeterministicCustomTransform>(variant_to_double(gain_it->second));
    });
}
...
TEST(DeterminismTest, SameInputSameOutputRk4) {
    ...
    for (int i = 0; i < 500; ++i) {
        ...
        EXPECT_DOUBLE_EQ(store1.read_value(temp_id1), store2.read_value(temp_id2)) << "Mismatch at tick " << i;
    }
}
...
TEST(DeterminismTest, NoDriftOver10kTicks) {
    ...
    EXPECT_NEAR(temp_final, temp_baseline, 0.01) << "Temperature drifted over 10k ticks";
}
