...
ParamValue matrix_param(std::initializer_list<std::initializer_list<double>> rows) {
    ParamArray matrix;
...
public:
    ConstantSignalModel(std::string id, SignalId output, double value, std::string unit)
        : id_(std::move(id)), output_(output), value_(value), unit_(std::move(unit)) {}

    void tick(double /*dt*/, SignalStore &store) override { store.write(output_, value_, unit_); }

    void reset() override {}

    double compute_stability_limit() const override { return std::numeric_limits<double>::infinity(); }
...
TEST(GraphCompilerTest, RegisterTransformFactoryRejectsInvalidInputs) {
    ...
    EXPECT_THROW(GraphCompiler::register_transform_factory(
                     "", [](const TransformSpec &) { return std::make_unique<AffineTestTransform>(0.0); }),
                 std::invalid_argument);
    EXPECT_THROW(GraphCompiler::register_transform_factory("test.empty_factory", empty_factory), std::invalid_argument);
}
...
TEST(GraphCompilerTest, RegisterTransformFactorySupportsExternalTypes) {
    ...
    if (!GraphCompiler::is_transform_registered(type)) {
        GraphCompiler::register_transform_factory(type, [](const TransformSpec &spec) -> std::unique_ptr<ITransform> {
            auto bias_it = spec.params.find("bias");
            if (bias_it == spec.params.end()) {
                throw std::runtime_error("Missing parameter: bias");
            }
            return std::make_unique<AffineTestTransform>(variant_to_double(bias_it->second, "transform[test]/bias"));
        });
    }
...
TEST(GraphCompilerTest, RegisterTransformFactoryRejectsDuplicateType) {
    ...
    if (!GraphCompiler::is_transform_registered(type)) {
        GraphCompiler::register_transform_factory(type, [](const TransformSpec &) -> std::unique_ptr<ITransform> {
            return std::make_unique<AffineTestTransform>(0.0);
        });
    }

    EXPECT_THROW(GraphCompiler::register_transform_factory(type,
...
TEST(GraphCompilerTest, ParseThermalMassModelWithNonPositiveHeatTransferCoeffThrows) {
    ModelSpec spec;
    spec.id = "chamber_air";
    spec.type = "thermal_mass";
    spec.params["thermal_mass"] = 1000.0;
...
TEST(GraphCompilerTest, ParseFirstOrderProcessModelWithInvalidIntegrationMethodThrows) {
    ModelSpec spec;
    spec.id = "pt1";
    spec.type = "first_order_process";
    spec.params["gain"] = 2.0;
...
TEST(GraphCompilerTest, ParseSecondOrderProcessModelWithInvalidIntegrationMethodThrows) {
    ModelSpec spec;
    spec.id = "pt2";
    spec.type = "second_order_process";
    spec.params["gain"] = 2.0;
...
TEST(GraphCompilerTest, ParseStateSpaceSisoDiscreteModel) {
    ...
    ASSERT_NE(model, nullptr);
    EXPECT_NE(model->describe().find("StateSpaceSisoDiscrete"), std::string::npos);
}
...
TEST(GraphCompilerTest, StrictModeRejectsUndeclaredStateSpaceSignalContracts) {
    ...
    EXPECT_THROW(compiler.compile(spec, signal_ns, func_ns, options), std::runtime_error);
}
...
TEST(GraphCompilerTest, ParseMassSpringDamperModelWithInvalidIntegrationMethodThrows) {
    ModelSpec spec;
    spec.id = "msd";
    spec.type = "mass_spring_damper";
    spec.params["mass"] = 1.0;
...
TEST(GraphCompilerTest, RegisterModelFactoryRejectsInvalidInputs) {
    ...
    EXPECT_THROW(GraphCompiler::register_model_factory(
                     "", [](const ModelSpec &, SignalNamespace &) -> std::unique_ptr<IModel> { return nullptr; }),
                 std::invalid_argument);
    EXPECT_THROW(GraphCompiler::register_model_factory("test.empty_model_factory", empty_factory),
                 std::invalid_argument);
}
...
TEST(GraphCompilerTest, RegisterModelFactorySupportsExternalTypes) {
    ...
    if (!GraphCompiler::is_model_registered(type)) {
        GraphCompiler::register_model_factory(
            type, [](const ModelSpec &spec, SignalNamespace &ns) -> std::unique_ptr<IModel> {
                auto output_it = spec.params.find("output_signal");
                auto value_it = spec.params.find("value");
                auto unit_it = spec.params.find("unit");
                if (output_it == spec.params.end() || value_it == spec.params.end() || unit_it == spec.params.end()) {
                    throw std::runtime_error("Missing required parameters for constant model");
                }

                const std::string output_path = variant_to_string(output_it->second, "model[test]/output_signal");
                const double value = variant_to_double(value_it->second, "model[test]/value");
                const std::string unit = variant_to_string(unit_it->second, "model[test]/unit");
                const SignalId output_id = ns.intern(output_path);

                return std::make_unique<ConstantSignalModel>(spec.id, output_id, value, unit);
            });
    }
...
TEST(GraphCompilerTest, RegisterModelFactoryRejectsDuplicateType) {
    ...
    if (!GraphCompiler::is_model_registered(type)) {
        GraphCompiler::register_model_factory(
            type, [](const ModelSpec &, SignalNamespace &) -> std::unique_ptr<IModel> {
                return std::make_unique<ConstantSignalModel>("dup", INVALID_SIGNAL, 0.0, "dimensionless");
            });
    }
    ...
    EXPECT_THROW(GraphCompiler::register_model_factory(
                     ...
                     [](const ModelSpec &, SignalNamespace &) -> std::unique_ptr<IModel> {
                         return std::make_unique<ConstantSignalModel>("dup2", INVALID_SIGNAL, 0.0, "dimensionless");
                     }),
                 std::runtime_error);
}
...
TEST(GraphCompilerTest, CustomModelWithInvalidOutputSignalIdThrows) {
    ...
    if (!GraphCompiler::is_model_registered(type)) {
        GraphCompiler::register_model_factory(
            type, [](const ModelSpec &, SignalNamespace &) -> std::unique_ptr<IModel> {
                return std::make_unique<ConstantSignalModel>("invalid", INVALID_SIGNAL, 1.0, "dimensionless");
            });
    }
...
TEST(GraphCompilerTest, StabilityValidationWithExpectedDt) {
    ...
    EXPECT_THROW(compiler.compile(spec, signal_ns, func_ns, 0.1), std::runtime_error);
}
...
TEST(GraphCompilerTest, StrictModeRejectsUnsignedCustomModel) {
    ...
    if (!GraphCompiler::is_model_registered(type)) {
        GraphCompiler::register_model_factory(
            type, [](const ModelSpec &, SignalNamespace &ns) -> std::unique_ptr<IModel> {
                const SignalId id = ns.intern("unsigned.model.output");
                return std::make_unique<ConstantSignalModel>("unsigned", id, 1.0, "dimensionless");
            });
    }
...
TEST(GraphCompilerTest, StrictModeAllowsSignedCustomModel) {
    ...
    if (!GraphCompiler::is_model_registered(type)) {
        ...
        GraphCompiler::register_model_factory_with_signature(
            type,
            [](const ModelSpec &model_spec, SignalNamespace &ns) -> std::unique_ptr<IModel> {
                ...
                const SignalId output_id = ns.intern(variant_to_string(output_it->second, "model/output_signal"));
                return std::make_unique<ConstantSignalModel>(model_spec.id, output_id, 5.0, "dimensionless");
            },
            signature);
    }
...
TEST(GraphCompilerTest, StrictModeEnforcesStructuredParamValidatorHook) {
    ...
    if (!GraphCompiler::is_model_registered(type)) {
        ModelSignature signature;
        signature.signal_param_units["output_signal"] = "dimensionless";
        signature.structured_param_validator = [](const ModelSpec &model_spec, bool strict,
                                                  const CompilationOptions &options) {
            (void)options;
            if (model_spec.params.find("structured_block") == model_spec.params.end()) {
                const std::string message =
                    "Missing structured parameter 'structured_block' for model '" + model_spec.id + "'";
                if (strict) {
...
TEST(GraphCompilerTest, PermissiveModeAllowsStructuredParamValidatorWarnings) {
    ...
    options.warning_handler = [&warnings](const std::string &warning) { warnings.push_back(warning); };
...
TEST(GraphCompilerTest, StrictModeRejectsSignedCustomModelMissingRequiredScalarParameter) {
    const std::string type = "test.strict_scalar_required_model";
    if (!GraphCompiler::is_model_registered(type)) {
        ModelSignature signature;
        signature.signal_param_units["output_signal"] = "dimensionless";
        signature.scalar_param_signatures["gain"] =
            ScalarParamSignature{"dimensionless", ScalarConstraint::greater_than(0.0), true};
        GraphCompiler::register_model_factory_with_signature(
            type,
            [](const ModelSpec &model_spec, SignalNamespace &ns) -> std::unique_ptr<IModel> {
                const auto output_it = model_spec.params.find("output_signal");
    ...
    try {
        ...
        EXPECT_NE(std::string(e.what()).find("scalar parameter 'gain'"), std::string::npos);
    }
}

TEST(GraphCompilerTest, StrictModeRejectsSignedCustomModelScalarConstraintViolation) {
...
TEST(GraphCompilerTest, StrictModeAllowsSignedCustomModelWithValidScalarConstraint) {
...
TEST(GraphCompilerTest, PermissiveModeEmitsLinearBoundaryWarning) {
    ...
    CompilationOptions options;
    options.dimensional_policy = DimensionalPolicy::permissive;
    options.warning_handler = [&warnings](const std::string &message) { warnings.push_back(message); };
...
TEST(GraphCompilerTest, PermissiveModeWarnsOnSignedCustomModelScalarConstraintViolation) {
