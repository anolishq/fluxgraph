...
FluxGraphServiceImpl::~FluxGraphServiceImpl() { std::cout << "[FluxGraph] Service shutdown\n"; }

// ============================================================================
// LoadConfig RPC
// ============================================================================

grpc::Status FluxGraphServiceImpl::LoadConfig(grpc::ServerContext * /*context*/,
    ...
    try {
        ...
        if (!request->config_hash().empty() && request->config_hash() == current_config_hash_) {
        ...
        if (request->format() == "yaml") {
            ...
            return grpc::Status(grpc::StatusCode::UNIMPLEMENTED, "YAML support not enabled");
#endif
            ...
            return grpc::Status(grpc::StatusCode::UNIMPLEMENTED, "JSON support not enabled");
#endif
        } else {
            response->set_success(false);
            response->set_error_message("Unknown format: " + request->format() + " (must be 'yaml' or 'json')");
        ...
        for (const auto &model : spec.models) {
            if (model.type == "thermal_mass") {
                ...
                if (temp_it != model.params.end() && std::holds_alternative<std::string>(temp_it->second)) {
        ...
        std::cout << "[FluxGraph] Config loaded: " << spec.models.size() << " models, " << spec.edges.size()
                  << " edges, " << spec.rules.size() << " rules, dt=" << dt_ << "s\n";
...
grpc::Status FluxGraphServiceImpl::RegisterProvider(grpc::ServerContext * /*context*/,
    ...
    if (!loaded_) {
        ...
        return grpc::Status(grpc::StatusCode::FAILED_PRECONDITION, "Config not loaded");
    }

    if (request->provider_id().empty()) {
        response->set_success(false);
        response->set_error_message("provider_id must be non-empty");
        return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "provider_id must be non-empty");
    }

    std::vector<std::string> requested_devices(request->device_ids().begin(), request->device_ids().end());
    ...
    for (const auto &[existing_session_id, existing_session] : sessions_) {
        if (existing_session.provider_id == request->provider_id()) {
            response->set_success(false);
            response->set_error_message("provider_id already registered: " + request->provider_id());
            return grpc::Status(grpc::StatusCode::ALREADY_EXISTS, "provider_id already registered");
        }

        for (const auto &device_id : requested_devices) {
            const auto found =
                std::find(existing_session.device_ids.begin(), existing_session.device_ids.end(), device_id);
            if (found != existing_session.device_ids.end()) {
                response->set_success(false);
                response->set_error_message("device_id already owned by another provider: " + device_id);
                return grpc::Status(grpc::StatusCode::ALREADY_EXISTS, "device_id ownership conflict");
            }
        }
    }
    ...
    std::cout << "[FluxGraph] Provider registered: " << request->provider_id() << " (session: " << session_id << ")\n";

    return grpc::Status::OK;
}

grpc::Status FluxGraphServiceImpl::UnregisterProvider(grpc::ServerContext * /*context*/,
    ...
    if (request->session_id().empty()) {
        ...
        return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "session_id must be non-empty");
    }
    ...
    response->set_success(true);
    std::cout << "[FluxGraph] Provider unregistered: " << provider_id << " (session: " << request->session_id()
              << ")\n";
...
grpc::Status FluxGraphServiceImpl::UpdateSignals(grpc::ServerContext * /*context*/,
    ...
    if (session_it == sessions_.end()) {
        return grpc::Status(grpc::StatusCode::UNAUTHENTICATED, "Invalid session_id - call RegisterProvider first");
    }
    ...
    for (const auto &sig : request->signals()) {
        ...
        if (id == INVALID_SIGNAL) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "Unknown signal: " + sig.path());
        }
    ...
    for (const auto &[session_id, session] : sessions_) {
        if (!session.last_tick_generation.has_value() || session.last_tick_generation.value() < current_generation) {
            all_ready = false;
            break;
        }
    }
    ...
    if (all_ready) {
        ...
        if (current_tick == 0 || (current_tick % 100 == 0 && current_tick != last_logged_tick) || (current_tick < 10)) {
            std::cout << "[FluxGraph] Tick " << current_tick << " (t=" << std::fixed << std::setprecision(1)
                      << sim_time_ << "s, generation=" << tick_generation_ << ", commands=" << response->commands_size()
                      << ")\n";
        ...
        bool ticked = tick_cv_.wait_for(lock, std::chrono::milliseconds(2000),
                                        [this, current_generation]() { return tick_generation_ > current_generation; });
        ...
        if (ticked) {
            ...
            std::cerr << "[FluxGraph] WARNING: " << provider_id
                      << " timed out waiting for tick (generation=" << current_generation << ", waited "
                      << wait_duration << "ms)\n";
...
grpc::Status FluxGraphServiceImpl::ReadSignals(grpc::ServerContext * /*context*/,
...
grpc::Status FluxGraphServiceImpl::Reset(grpc::ServerContext * /*context*/,
...
grpc::Status FluxGraphServiceImpl::Check(grpc::ServerContext * /*context*/,
...
void FluxGraphServiceImpl::convert_command(const fluxgraph::Command &cmd, fluxgraph::rpc::Command *pb_cmd) {
...
void FluxGraphServiceImpl::prune_stale_sessions_locked(const std::string &active_session_id,
    ...
    for (auto it = sessions_.begin(); it != sessions_.end();) {
        ...
        const auto age = std::chrono::duration_cast<std::chrono::milliseconds>(now - it->second.last_update);
        if (age > session_timeout_) {
            std::cerr << "[FluxGraph] Evicting stale provider session: provider_id=" << it->second.provider_id
                      << ", session_id=" << it->first << ", age_ms=" << age.count() << "\n";
...
std::vector<fluxgraph::Command> FluxGraphServiceImpl::filter_commands_for_session_locked(
...
void FluxGraphServiceImpl::populate_tick_response_for_session_locked(const std::string &session_id,
                                                                     fluxgraph::rpc::TickResponse *response) {
    response->set_tick_occurred(true);
    response->set_sim_time_sec(last_completed_sim_time_);

    auto provider_commands = filter_commands_for_session_locked(session_id, last_completed_commands_);
